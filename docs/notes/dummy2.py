...
\subsubsection*{Decoded action loss and residual action generation}
The coefficient FM heads predict vector fields in normalized coefficient space. To connect the coefficient objective to the actual micro-actions executed by the policy, we recover a one-step clean coefficient estimate during training,
\begin{equation}
...
\right).
\end{equation}
Here $\mathcal{D}_{\Phi}$ means: decode cumulative translation and finite-difference it; decode command-space cumulative rotation and recover relative rotation-vector commands by adjacent $SO(3)$ differences; and decode the absolute gripper command directly.

We add a learned full-horizon residual correction on top of this smooth B-spline backbone. Let the final coefficient-token hidden states be
\begin{equation}
H_{\mathrm{coef}}
=
...
\end{equation}
The residual mixer first refines these coefficient-token features with a pre-normalized FFN,
\begin{equation}
\bar H
=
H_{\mathrm{coef}}
...
\end{equation}
then maps the coefficient-token axis to the action-time axis with a token-to-time FFN. Equivalently, for each hidden channel the same learned map
\begin{equation}
g_{\mathrm{tok}\to\mathrm{time}}:\mathbb{R}^{3K}\rightarrow\mathbb{R}^{D_{\mathrm{fit}}}
\end{equation}
converts coefficient-token activations into $D_{\mathrm{fit}}$ action-time features, producing
\begin{equation}
Z\in\mathbb{R}^{B\times D_{\mathrm{fit}}\times H}.
\end{equation}
We add learned action-time embeddings
\begin{equation}
Z_{b,\tau,:}\leftarrow Z_{b,\tau,:}+e_{\tau}^{\mathrm{res}},
\qquad
\tau=1,\ldots,D_{\mathrm{fit}},
\end{equation}
where $e_{\tau}^{\mathrm{res}}\in\mathbb{R}^{H}$ is a learned embedding for the $\tau$-th decoded action slot. A second pre-normalized FFN refines the action-time features,
\begin{equation}
\widetilde Z
=
Z+
\alpha_t f_t\!\left(\operatorname{LN}(Z)\right),
\end{equation}
and an action-coordinate projection produces a raw residual proposal
\begin{equation}
R_{\theta}=W_{\mathrm{res}}\widetilde Z
\in\mathbb{R}^{B\times D_{\mathrm{fit}}\times 7}.
\end{equation}
The final residual is bounded with a per-coordinate scale vector $s_{\mathrm{ACTION}}$, initialized from the dataset ACTION standard deviation:
\begin{equation}
\Delta \hat A^{\mathrm{res}}
=
s_{\mathrm{ACTION}}\odot\tanh(R_{\theta}).
\end{equation}
The last residual projection is zero-initialized so the initial residual branch is exactly zero. Thus, at the start of finetuning, the policy is the original B-spline coefficient policy; the residual learns only corrections that reduce the decoded action loss. The final decoded action chunk used for training and inference is
\begin{equation}
...
The decoded action loss is applied only to this final action chunk. We do not add a separate base-decoded loss or residual regularizer in the first implementation; the coefficient FM objective keeps the B-spline backbone grounded, while zero initialization and bounded residual scale prevent the residual branch from dominating at initialization. Let $R_s$ be the number of real target actions available from start $s$ before the episode ends. For valid real actions, define time-region weights
\begin{equation}
w_{s,\tau}=\begin{cases}
...
\end{equation}
The first residual experiment uses
\begin{equation}
...
\end{equation}
The total Experiment 3 objective with decoded residual learning is
\begin{align}
L_s^{\mathrm{exp3}}=
...
\end{align}
In the current sweep, $\lambda_{\mathrm{dec}}$ is grid-searched, e.g. $\{0.5,1.0,1.5,2.0\}$.
...
# Full-horizon learned residual mixer.
H_coef = cat([tr_h, rot_h, grip_h], dim=1)       # [B, 3K, H]
H = H_coef + alpha_c * coeff_ffn(layer_norm(H_coef))
Z = token_to_time_ffn(H.transpose(1, 2)).transpose(1, 2)  # [B, Dfit, H]
Z = Z + learned_action_time_embedding[:, :Dfit]
Z = Z + alpha_t * action_time_ffn(layer_norm(Z))
R = residual_out_proj(Z)                         # [B, Dfit, 7]
Delta_A = ACTION_STD * torch.tanh(R)             # zero-init residual_out_proj
