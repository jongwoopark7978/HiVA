...
class SmolVLAConfig(PreTrainedConfig):
    ...
    # Duration-only HiVA-lite.
    # Adds one learned duration token after the action tokens in the action expert.
    # The raw action chunk path stays unchanged. `n_action_steps` is the queue capacity,
    # while the predicted duration controls how many prefix actions are executed before re-observing.
    ...
    def __post_init__(self):
        ...
        if self.use_duration_head:
            if len(self.duration_classes) < 2:
                raise ValueError("`duration_classes` must contain at least 2 values.")
            if any(d <= 0 for d in self.duration_classes):
                raise ValueError("All `duration_classes` entries must be positive integers.")
            if tuple(sorted(set(self.duration_classes))) != tuple(self.duration_classes):
                raise ValueError("`duration_classes` must be strictly increasing.")
            if max(self.duration_classes) > self.chunk_size:
                raise ValueError(
                    "The largest duration class cannot exceed `chunk_size`. "
                    f"Got max(duration_classes)={max(self.duration_classes)} and chunk_size={self.chunk_size}."
                )
            if max(self.duration_classes) > self.n_action_steps:
                raise ValueError(
                    "The largest duration class cannot exceed `n_action_steps`, otherwise inference "
                    "would silently clamp the executed prefix. "
                    f"Got max(duration_classes)={max(self.duration_classes)} and n_action_steps={self.n_action_steps}."
                )
