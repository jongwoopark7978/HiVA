...
# stage0 LP-MT K=4 F=15 HiVA coefficient run, executed on bigtoken GPUs 4-7.
#
...
TRAIN_DIR="${TRAIN_DIR:-/nfs/bigcornea.cs.stonybrook.edu/add_disk3/jongwoopark/HiVA_train/finetuning_stage0/smolvla_hiva_coeff_lpmt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k4_f15_bigcornea_b64_s0p5_20260523_035135_k8_k4}"
...
HIVA_COEFF_SIDECAR="${HIVA_COEFF_SIDECAR:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k${HIVA_K}_f15_canonical_lp_mt.parquet}"
HIVA_COEFF_SUMMARY="${HIVA_COEFF_SUMMARY:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k${HIVA_K}_f15_canonical_lp_mt.summary.json}"
...
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/eval_stage0_lpmt_k${HIVA_K}_f15_all_ckpts_${GPU_GROUP}_50eps_${TIMESTAMP}.queue.log}"
...
run_checkpoint() {
  ...
  local label="stage0_lpmt_k${HIVA_K}_f15_ckpt_${ckpt}_50eps_bs${EVAL_BATCH_SIZE}"
...
main() {
  ...
  echo "===== stage0 LP-MT K${HIVA_K} F15 all-checkpoint full eval started at $(date) ====="
  ...
  echo "===== stage0 LP-MT K${HIVA_K} F15 all-checkpoint full eval finished at $(date) ====="
}
