...
# stage0 LP-MT K=10 F=15 P(tr,rot,grip)=3,3,5 HiVA coefficient run.
...
TRAIN_DIR="${TRAIN_DIR:-/nfs/bigcornea.cs.stonybrook.edu/add_disk3/jongwoopark/HiVA_train/finetuning_stage0/smolvla_hiva_coeff_lpmt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k10_f15_p3trrot_p5grip_bigbrain_b256_g2_s0p25_steps10000_20260519_222000_p5grip_after_p7_gpu8_9}"
...
SIDECAR_ROOT="${SIDECAR_ROOT:-/nfs/bigbrain.cs.stonybrook.edu/add_disk0/jongwoopark}"
DATA_ROOT="${DATA_ROOT:-${SIDECAR_ROOT}/libero_lerobot_v3_lerobotkeys}"
HIVA_COEFF_SIDECAR="${HIVA_COEFF_SIDECAR:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_p3trrot_p5grip_f15_canonical_lp_mt.parquet}"
HIVA_COEFF_SUMMARY="${HIVA_COEFF_SUMMARY:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_p3trrot_p5grip_f15_canonical_lp_mt.summary.json}"
...
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/eval_stage0_lpmt_k10_f15_p3trrot_p5grip_all_ckpts_${GPU_GROUP}_50eps_${TIMESTAMP}.queue.log}"
...
run_checkpoint() {
  ...
  local label="stage0_lpmt_k10_f15_p3trrot_p5grip_ckpt_${ckpt}_50eps_bs${EVAL_BATCH_SIZE}"
...
main() {
  ...
  echo "===== stage0 LP-MT K10 F15 p3trrot p5grip all-checkpoint full eval started at $(date) ====="
  ...
  echo "===== stage0 LP-MT K10 F15 p3trrot p5grip all-checkpoint full eval finished at $(date) ====="
}
