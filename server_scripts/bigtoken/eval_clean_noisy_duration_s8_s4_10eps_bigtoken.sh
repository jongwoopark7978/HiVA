...
# Simple full LIBERO eval for clean/noisy duration SmolVLA S=8 and S=4
# checkpoints on bigtoken.
#
# Each checkpoint evaluates all 4 LIBERO suites, task_ids [0..9], with
# N_EPISODES=10 per task. Within a checkpoint, suites run in parallel on
# GPU_IDS=0,1,2,3 by default. Checkpoints run sequentially.
#
...
#
# Example:
#   nohup setsid bash server_scripts/bigtoken/eval_clean_noisy_duration_s8_s4_10eps_bigtoken.sh \
#     > outputs/eval_logs/eval_clean_noisy_duration_s8_s4_10eps_$(date +%Y%m%d_%H%M%S).outer.log 2>&1 < /dev/null &
...
S8_POLICY_PATH="${S8_POLICY_PATH:-/home/jongwoopark/lerobot/outputs/train/smolvla_hiva_duration_clean_noisy_bigflow_s8_20260503_164505/checkpoints/last/pretrained_model}"
S4_POLICY_PATH="${S4_POLICY_PATH:-/home/jongwoopark/lerobot/outputs/train/smolvla_hiva_duration_clean_noisy_bigflow_s4_20260503_103525/checkpoints/last/pretrained_model}"
...
echo "MAX_EPISODES_RENDERED=${MAX_EPISODES_RENDERED}"
echo "N_ACTION_STEPS=${N_ACTION_STEPS}"
echo "S8_POLICY_PATH=${S8_POLICY_PATH}"
echo "S4_POLICY_PATH=${S4_POLICY_PATH}"

run_checkpoint \
  "smolvla_hiva_duration_clean_noisy_bigflow_s8_20260503_164505_10eps" \
  "${S8_POLICY_PATH}"

run_checkpoint \
  "smolvla_hiva_duration_clean_noisy_bigflow_s4_20260503_103525_10eps" \
  "${S4_POLICY_PATH}"

echo "All clean/noisy duration S8/S4 10-episode evaluations finished at $(date)."
