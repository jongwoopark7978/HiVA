...
# Full 50-episode LIBERO eval for all saved checkpoints from the BigBrain
# stage0 MT K=10 F=10 HiVA coefficient run, executed on bigtoken GPUs 0-3.
#
# "Full" here means:
#   - all 4 LIBERO suites: object, goal, spatial, libero_10
#   - task_ids [0..9] for each suite
#   - 50 episodes per task = 2000 episodes per checkpoint
#   - one rendered episode per task = 40 videos per checkpoint
#   - staged LIBERO-10 fanout: GPU3 runs tasks [0..6] while GPUs 0-2 run the
#     short suites, then GPUs 0-2 run LIBERO-10 tasks [7], [8], [9].
...
TRAIN_DIR="${TRAIN_DIR:-/nfs/bigcornea.cs.stonybrook.edu/add_disk3/jongwoopark/HiVA_train/finetuning_stage0/smolvla_hiva_coeff_mt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k10_f10_bigbrain_b256_g2_s0p5_steps5000_20260519_212637_resume_gpu6_7_f10}"
...
HIVA_COEFF_SIDECAR="${HIVA_COEFF_SIDECAR:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_f10_canonical_lp_mt.parquet}"
HIVA_COEFF_SUMMARY="${HIVA_COEFF_SUMMARY:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_f10_canonical_lp_mt.summary.json}"
...
SWEEP_OUTPUT_DIR="${SWEEP_OUTPUT_DIR:-${REPO_ROOT}/outputs/eval/full_bigtoken_${MODEL_TAG}_all_ckpts_50eps_bs${EVAL_BATCH_SIZE}_${TIMESTAMP}}"
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/eval_stage0_mt_k10_f10_all_ckpts_gpu0_3_50eps_${TIMESTAMP}.queue.log}"
...
run_checkpoint() {
  ...
  # Keep this compact because the lower-level helper embeds it in log filenames.
  local label="stage0_mt_k10_f10_ckpt_${ckpt}_50eps_bs${EVAL_BATCH_SIZE}"
...
main() {
  ...
  echo "===== stage0 MT K10 F10 all-checkpoint full eval started at $(date) ====="
  ...
  echo "===== stage0 MT K10 F10 all-checkpoint full eval finished at $(date) ====="
}
