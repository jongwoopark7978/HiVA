...
# duration flow matching on bigflow.
#
# This script is intentionally MT-named so the continuous-duration MT jobs no
# longer need to go through the older HP-named helper. It uses an existing MT
# coefficient sidecar. No separate duration sidecar is needed for bounded or
# mean/std continuous duration normalization when the target is duration_label.
#
# Example smoke:
#   GPU_IDS=1 NUM_GPUS=1 NUM_PROCESSES=1 BATCH_PER_GPU=2 STEPS=2 SAVE_FREQ=1 \
#   HIVA_DURATION_CONT_NORM=bounded HIVA_SUFFIX_ATTENTION=duration_reads_coeffs WANDB_ENABLE=false \
#   bash server_scripts/bigflow/finetune_bigflow_ckpt_20k_hiva_coeff_mt_continuous.sh
#
# Example full-ish launch:
#   GPU_IDS=4,5,6,7 NUM_GPUS=4 BATCH_PER_GPU=160 S=2 WANDB_ENABLE=true \
#   HIVA_DURATION_CONT_NORM=bounded HIVA_SUFFIX_ATTENTION=duration_reads_coeffs \
#   bash server_scripts/bigflow/finetune_bigflow_ckpt_20k_hiva_coeff_mt_continuous.sh
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/finetune_bigflow_hiva_coeff_mt_continuous_$(date +%Y%m%d_%H%M%S).log"
...
SIDECAR="${SIDECAR:-/nfs/bigflow/add_disk0/jongwoopark/libero_hiva_coeff_sidecar_d2_15_w1_10_w3_0_k${HIVA_K}_canonical_mt.parquet}"
SIDECAR_SUMMARY="${SIDECAR_SUMMARY:-/nfs/bigflow/add_disk0/jongwoopark/libero_hiva_coeff_sidecar_d2_15_w1_10_w3_0_k${HIVA_K}_canonical_mt.summary.json}"
