...
#   bash server_scripts/bigflow/finetune_bigflow_ckpt_20k_hiva_coeff.sh
#
# Observed HiVA coefficient SmolVLA per-GPU memory on bigflow RTX 6000 Ada:
...
#
# Smoke-test override:
#   GPU_IDS=0 NUM_GPUS=1 NUM_PROCESSES=1 BATCH_PER_GPU=2 STEPS=1 SAVE_FREQ=1 \
#   bash server_scripts/bigflow/finetune_bigflow_ckpt_20k_hiva_coeff.sh
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/finetune_bigflow_hiva_coeff_$(date +%Y%m%d_%H%M%S).log"
...
HIVA_DURATION_NOISY_LOSS_WEIGHT="${HIVA_DURATION_NOISY_LOSS_WEIGHT:-0.1}"
HIVA_DURATION_NOISY_SIGMA="${HIVA_DURATION_NOISY_SIGMA:-0.25}"
HIVA_DURATION_LOSS="${HIVA_DURATION_LOSS:-mean}"
...
build_run_id
RUN_NAME="${RUN_NAME:-smolvla_hiva_coeff_bigflow_full_s${S}_${RUN_ID}}"
