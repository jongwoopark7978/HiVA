...
# Resume the interrupted bigflow queue after job1 completed:
#   job2: LP-MT, duration={2,15}, Dmax=15, preview horizon=20, K=10, S=2
#   job3: MT, duration={2,15}, K=10, S=1
#
# This keeps the same default recipe as the original queue:
#   BATCH_PER_GPU=160
...
RUN_STAMP="${RUN_STAMP:-20260506_232204}"
...
run_lpmt() {
  ...
  echo "===== Starting resumed job2 LP-MT HiVA S=${s} at $(date) ====="
  env "${COMMON_ENV[@]}" \
    S="${s}" \
  ...
  echo "===== Finished resumed job2 LP-MT HiVA S=${s} at $(date) ====="
}

run_mt() {
  local s="$1"

  echo "===== Starting resumed job3 MT HiVA S=${s} at $(date) ====="
  env "${COMMON_ENV[@]}" \
    S="${s}" \
  ...
  echo "===== Finished resumed job3 MT HiVA S=${s} at $(date) ====="
}
