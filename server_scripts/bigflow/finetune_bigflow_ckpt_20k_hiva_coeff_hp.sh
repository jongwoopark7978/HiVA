...
# HP-basis cleaner-suffix HiVA coefficient SmolVLA finetuning on bigflow.
#
# This variant uses the canonical hold-pad coefficient sidecar: every sample is fit
# with one Dmax x K B-spline basis, and duration only controls how many decoded
# actions are executed from the predicted macro.
# Default K is 10, giving suffix length 1 + 10 + 10 + 10 = 31 tokens.
#
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/finetune_bigflow_hiva_coeff_hp_$(date +%Y%m%d_%H%M%S).log"
