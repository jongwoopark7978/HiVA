...
# Sequential bigflow sweep for canonical HP K=10 HiVA coefficient SmolVLA with
# the residual-FFN duration head enabled for every run.
#
# Runs four jobs on GPUs 4-7 by default:
#   1) S=4, hiva_duration_loss=ce_mean, hiva_suffix_attention=full
#   2) S=4, hiva_duration_loss=ce_mean, hiva_suffix_attention=duration_prefix
#   3) S=2, hiva_duration_loss=ce_mean, hiva_suffix_attention=full
#   4) S=2, hiva_duration_loss=ce_mean, hiva_suffix_attention=duration_prefix
...
OUTER_LOG="${OUTER_LOG:-${LOG_DIR}/hiva_coeff_hp_k10_residual_ffn_attention_sweep_${SWEEP_TS}.outer.log}"
...
run_job() {
  ...
  local run_name
  run_name="smolvla_hiva_coeff_hp_k${HIVA_K}_residual_ffn_bigflow_${job_id}_${duration_loss}_${suffix_attention}_w${HIVA_DURATION_NOISY_LOSS_WEIGHT}_sigma${HIVA_DURATION_NOISY_SIGMA}_b${BATCH_PER_GPU}_s${step_reduction}_${SWEEP_TS}"
  run_name="${run_name//./p}"
  ...
  HIVA_DURATION_FFN_HIDDEN_MULT="${HIVA_DURATION_FFN_HIDDEN_MULT}" \
  HIVA_DURATION_FFN_ALPHA_INIT="${HIVA_DURATION_FFN_ALPHA_INIT}" \
...
{
  echo "HP HiVA K=10 residual-FFN attention sweep started at $(date)"
  echo "Host: $(hostname)"
  ...
  run_job "job1" 4 "ce_mean" "full" "batch-mean CE, full suffix attention"
  run_job "job2" 4 "ce_mean" "duration_prefix" "batch-mean CE, duration-prefix suffix attention"
  run_job "job3" 2 "ce_mean" "full" "batch-mean CE, full suffix attention"
  run_job "job4" 2 "ce_mean" "duration_prefix" "batch-mean CE, duration-prefix suffix attention"

  echo "HP HiVA K=10 residual-FFN attention sweep finished at $(date)"
} 2>&1 | tee -a "${OUTER_LOG}"
