...
# Fan out full 50-episode LIBERO evals for selected b256 S=0.5 stage0 LP-MT
# HiVA checkpoints on BigBrain GPU 6. Each checkpoint gets an independent
# runner process; task evals inside each checkpoint remain sequential.
...
CKPTS=(003125 003250 003375 003500)
...
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/eval_full_fanout_gpu${GPU_ID}_${MODEL_NAME}_ckpts_003125_003500_50eps_bs50_${TIMESTAMP}.queue.log}"
...
launch_ckpt() {
  local ckpt="$1"
  local outer_log="${LOG_DIR}/eval_full_fanout_gpu${GPU_ID}_${MODEL_NAME}_ckpt_${ckpt}_50eps_bs50_${TIMESTAMP}.outer.log"
  local runner_queue_log="${LOG_DIR}/eval_full_fanout_gpu${GPU_ID}_${MODEL_NAME}_ckpt_${ckpt}_50eps_bs50_${TIMESTAMP}.queue.log"
...
main() {
  ...
  echo "===== b256 S=0.5 full fanout eval started at $(date) ====="
  ...
  if [[ "${status}" -ne 0 ]]; then
    echo "One or more fanout full eval jobs failed." >&2
    exit "${status}"
  fi

  echo "===== b256 S=0.5 full fanout eval finished at $(date) ====="
}
