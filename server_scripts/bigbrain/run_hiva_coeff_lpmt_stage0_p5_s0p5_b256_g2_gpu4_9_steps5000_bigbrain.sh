...
# BigBrain stage-0 LP-MT HiVA coefficient finetune matching the P=5,
# no-residual/no-decoded-loss run, with per-GPU batch size 256 on GPUs 4,9.
...
SAVE_STEPS="${SAVE_STEPS:-[3125,3250,3375,3500,3625,3750,3875,4000,4375,5000]}"
RUN_NAME="${RUN_NAME:-smolvla_hiva_coeff_lpmt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k10_p5_f15_bigbrain_b256_g2_s0p5_steps5000_${RUN_STAMP}}"
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/hiva_coeff_lpmt_stage0_p5_s0p5_b256_g2_gpu4_9_steps5000_bigbrain_${RUN_STAMP}.log}"

SIDECAR_ROOT="${SIDECAR_ROOT:-/nfs/bigbrain/add_disk0/jongwoopark}"

echo "Launching P5 stage0 LP-MT finetune on BigBrain"
...
WANDB_NOTES="${WANDB_NOTES:-P5 stage0 LP-MT BigBrain run; GPUs=4,9; batch_per_gpu=256; effective_batch=512; steps=5000; save_steps=${SAVE_STEPS}}" \
