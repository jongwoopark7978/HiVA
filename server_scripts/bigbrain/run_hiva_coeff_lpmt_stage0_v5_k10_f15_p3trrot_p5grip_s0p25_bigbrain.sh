...
HIVA_DEGREE_GRIP="${HIVA_DEGREE_GRIP:-5}"
...
RUN_NAME="${RUN_NAME:-smolvla_hiva_coeff_lpmt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k10_f15_p3trrot_p5grip_bigbrain_b${BATCH_PER_GPU}_g${NUM_PROCESSES}_s0p25_steps${TOTAL_STEPS}_${RUN_STAMP}}"
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/hiva_coeff_lpmt_stage0_v5_k10_f15_p3trrot_p5grip_s0p25_bigbrain_${RUN_STAMP}.log}"

SIDECAR="${SIDECAR:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_p3trrot_p5grip_f15_canonical_lp_mt.parquet}"
SIDECAR_SUMMARY="${SIDECAR_SUMMARY:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_p3trrot_p5grip_f15_canonical_lp_mt.summary.json}"

echo "Launching mixed-degree LP-MT HiVA: K=10 F=15 P(tr,rot,grip)=3,3,5"
...
WANDB_NOTES="${WANDB_NOTES:-Mixed-degree LP-MT HiVA K=10 F=15 D=4,6,10 P_tr=3 P_rot=3 P_grip=5; S=0.25; batch_per_gpu=${BATCH_PER_GPU}; GPUs=${GPU_IDS}; steps=${TOTAL_STEPS}; save_steps=${SAVE_STEPS}}" \
