...
# Fan out all incomplete BigBrain residual-flow checkpoint eval resumes across
# GPUs 6 and 7. Each checkpoint runner skips task dirs that already contain
# eval_info.json and only visits the known missing suite/task chunks.
...
mkdir -p "${LOG_DIR}"
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/resume_crashed_residual_evals_fanout_gpu6_7_bigbrain_bs${EVAL_BATCH_SIZE}_${TIMESTAMP}.queue.log}"
...
launch_resume_chunk() {
  ...
  local outer_log="${LOG_DIR}/resume_fanout_${label}_ckpt_${ckpt}_gpu${gpu_id}_${TIMESTAMP}.outer.log"

  require_dir "${train_dir}/checkpoints/${ckpt}/pretrained_model"
  require_dir "${eval_dir}"

  echo "===== $(date) launching fanout ${label} ckpt_${ckpt} on GPU ${gpu_id} ====="
...
main() {
  ...
  echo "===== fanout resume crashed residual evals started at $(date) ====="
  echo "TIMESTAMP=${TIMESTAMP}"
  echo "EVAL_BATCH_SIZE=${EVAL_BATCH_SIZE}"
  echo "QUEUE_LOG=${QUEUE_LOG}"

  local pids=()
  launch_resume_chunk "daw2p0" "6" "${DAW2_TRAIN}" "000469" "${DAW2_EVAL}" \
    "libero_10" "${EMPTY_IDS}" "${EMPTY_IDS}" "${EMPTY_IDS}" "${ALL_IDS}"
  pids+=("$!")

  launch_resume_chunk "trrot4" "7" "${TRROT4_TRAIN}" "000209" "${TRROT4_EVAL}" \
    "libero_spatial,libero_10" "${EMPTY_IDS}" "${EMPTY_IDS}" "${SPATIAL_8_9}" "${ALL_IDS}"
  pids+=("$!")

  launch_resume_chunk "trrot2" "6" "${TRROT2_TRAIN}" "000021" "${TRROT2_EVAL}" \
    "libero_object,libero_goal,libero_spatial,libero_10" "${OBJ_8_9}" "${ALL_IDS}" "${ALL_IDS}" "${ALL_IDS}"
  pids+=("$!")

  launch_resume_chunk "trrot2" "7" "${TRROT2_TRAIN}" "000209" "${TRROT2_EVAL}" \
    "libero_10" "${EMPTY_IDS}" "${EMPTY_IDS}" "${EMPTY_IDS}" "${ALL_IDS}"
  pids+=("$!")

  launch_resume_chunk "trrot5" "6" "${TRROT5_TRAIN}" "000157" "${TRROT5_EVAL}" \
    "libero_spatial,libero_10" "${EMPTY_IDS}" "${EMPTY_IDS}" "${SPATIAL_8_9}" "${ALL_IDS}"
  pids+=("$!")

  launch_resume_chunk "trrot5" "7" "${TRROT5_TRAIN}" "000209" "${TRROT5_EVAL}" \
    "libero_object,libero_goal,libero_spatial,libero_10" "${OBJ_8_9}" "${ALL_IDS}" "${ALL_IDS}" "${ALL_IDS}"
  pids+=("$!")
  ...
  if [[ "${status}" -ne 0 ]]; then
    echo "One or more fanout resume chunks failed." >&2
    exit "${status}"
  fi

  echo "===== fanout resume crashed residual evals finished at $(date) ====="
}
