...
# Case 1: K=10, F=10, D={4,6,10}. Since F == dmax, this removes the LP-MT
# preview tail while preserving the same executable duration classes.
#
...
HIVA_K="${HIVA_K:-10}"
HIVA_FIT_HORIZON="${HIVA_FIT_HORIZON:-10}"
...
RUN_NAME="${RUN_NAME:-smolvla_hiva_coeff_mt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k10_f10_bigbrain_b${BATCH_PER_GPU}_g${NUM_PROCESSES}_s0p5_steps${TOTAL_STEPS}_${RUN_STAMP}}"
QUEUE_LOG="${QUEUE_LOG:-${LOG_DIR}/hiva_coeff_lpmt_stage0_v5_k10_f10_s0p25_bigbrain_${RUN_STAMP}.log}"

SIDECAR="${SIDECAR:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_f10_canonical_lp_mt.parquet}"
SIDECAR_SUMMARY="${SIDECAR_SUMMARY:-${SIDECAR_ROOT}/libero_hiva_coeff_sidecar_v5_d4_6_10_wide_commit6_k10_f10_canonical_lp_mt.summary.json}"

echo "Launching LP-MT preview ablation case1: K=10 F=10 no-preview"
...
if [[ ! -e "${SIDECAR}" || ! -e "${SIDECAR_SUMMARY}" ]]; then
  echo "ERROR: required K=10 F=10 sidecar files are missing." >&2
  echo "  ${SIDECAR}" >&2
...
WANDB_NOTES="${WANDB_NOTES:-MT HiVA preview ablation case1 K=10 F=10 no-preview; D=4,6,10; S=0.5; batch_per_gpu=${BATCH_PER_GPU}; GPUs=${GPU_IDS}; steps=${TOTAL_STEPS}; save_steps=${SAVE_STEPS}}" \
