...
# Bigcornea-only MT coefficient-HiVA job2.
#
# This is the clean-duration-loss rerun from the new-methods sweep:
#   hiva_suffix_attention=full
#   hiva_duration_clean_loss_weight=1.0
#   hiva_duration_head_type=residual_ffn
#   hiva_duration_loss=ce_mean
#   duration classes=[6,10,15], canonical MT sidecar
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/finetune_bigcornea_mt_hiva_coeff_k10_job2_clean_duration_$(date +%Y%m%d_%H%M%S).log"
...
HIVA_DURATION_CLEAN_LOSS_WEIGHT="${HIVA_DURATION_CLEAN_LOSS_WEIGHT:-1.0}"
...
build_run_id
DEFAULT_RUN_NAME="smolvla_hiva_coeff_mt_k${HIVA_K}_bigcornea_job2_${HIVA_DURATION_HEAD_TYPE}_${HIVA_DURATION_LOSS}_${HIVA_SUFFIX_ATTENTION}_clean${HIVA_DURATION_CLEAN_LOSS_WEIGHT}_w${HIVA_DURATION_NOISY_LOSS_WEIGHT}_sigma${HIVA_DURATION_NOISY_SIGMA}_b${BATCH_PER_GPU}_s${S}_${RUN_ID}"
