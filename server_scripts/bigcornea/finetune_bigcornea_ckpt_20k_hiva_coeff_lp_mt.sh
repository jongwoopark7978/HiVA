...
# Canonical long-preview max-target (LP-MT) HiVA coefficient SmolVLA finetuning on bigcornea.
#
# LP-MT fits a preview chunk with one canonical basis, while duration selects
# only the executable prefix from HIVA_DURATION_CLASSES.
...
SIDECAR="${SIDECAR:-/nfs/bigcornea/add_disk2/jongwoopark/libero_hiva_coeff_sidecar_d2_15_w1_10_w3_0_k${HIVA_K}_f${HIVA_FIT_HORIZON}_canonical_lp_mt.parquet}"
SIDECAR_SUMMARY="${SIDECAR_SUMMARY:-/nfs/bigcornea/add_disk2/jongwoopark/libero_hiva_coeff_sidecar_d2_15_w1_10_w3_0_k${HIVA_K}_f${HIVA_FIT_HORIZON}_canonical_lp_mt.summary.json}"

if [[ -z "${RUN_NAME:-}" ]]; then
  RUN_NAME="smolvla_hiva_coeff_lpmt_${HIVA_DURATION_HEAD_TYPE}_${HIVA_SUFFIX_ATTENTION}_${HIVA_DURATION_LOSS}_d2_15_k${HIVA_K}_f${HIVA_FIT_HORIZON}_bigcornea_b${BATCH_PER_GPU:-64}_s${S:-2}_$(date +%Y%m%d_%H%M%S)"
fi
...
export POLICY_CHUNK_SIZE
export POLICY_N_ACTION_STEPS
export DATA_ROOT
export SIDECAR
