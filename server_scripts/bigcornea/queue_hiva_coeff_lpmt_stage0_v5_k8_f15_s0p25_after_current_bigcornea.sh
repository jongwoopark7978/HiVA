...
# Queue the K=8, F=15 LP-MT stage-0 HiVA run with S=0.25 behind the current
# BigCornea all-GPU training job. This reuses the K sweep launcher but
# restricts it to K=8 and writes checkpoints directly to add_disk3.
...
S="${S:-0.25}"
S_TAG="${S//./p}"
RUN_NAME="${RUN_NAME:-smolvla_hiva_coeff_lpmt_stage0_v5_d4_6_10_coeffpool_full_ce_mean_k8_f15_bigcornea_b64_s${S_TAG}_${RUN_STAMP}}"
...
mkdir -p "${LOG_DIR}"
OUTER_LOG="${OUTER_LOG:-${LOG_DIR}/queue_hiva_coeff_lpmt_stage0_v5_k8_f15_s0p25_after_current_bigcornea_${RUN_STAMP}.outer.log}"
exec > >(tee -a "${OUTER_LOG}") 2>&1

WAIT_FOR_SESSION="${WAIT_FOR_SESSION:-hiva_k6_f15_s0p25_queue}"
...
echo "K=8 F=15 S=0.25 BigCornea queue started at $(date)"
...
echo "K=8 F=15 S=0.25 BigCornea queue finished at $(date)"
