...
# Standalone stage-1 residual-only finetune for LP-MT HiVA v5 on bigcornea
...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/run_hiva_residual_stage1_xattn_bigcornea_${RUN_STAMP}.log}"
...
RUN_NAME="${RUN_NAME:-smolvla_hiva_coeff_lpmt_residual_stage1_xattn_b${HIVA_RESIDUAL_NUM_BLOCKS}_v5_d4_6_10_k${HIVA_K}_f${HIVA_FIT_HORIZON}_scale${HIVA_RESIDUAL_SCALE_MULT//./p}_daw${HIVA_DECODED_ACTION_LOSS_WEIGHT//./p}_b${BATCH_PER_GPU}_s${S}_${RUN_STAMP}}"
...
# Keep the residual-only sitecustomize out of shell-side helper Python snippets.
# It intentionally imports the policy module when HIVA_TRAIN_RESIDUAL_ONLY=true,
# which is only needed for the actual lerobot-train process.
...
printf 'OUTPUT_DIR=%s\n' "${OUTPUT_DIR}"
printf 'LOG_FILE=%s\n' "${LOG_FILE}"
printf 'INIT_HIVA_BASE=%s\n' "${INIT_HIVA_BASE}"
printf 'DATA_ROOT=%s\n' "${DATA_ROOT}"
printf 'SIDECAR=%s\n' "${SIDECAR}"
printf 'SIDECAR_SUMMARY=%s\n' "${SIDECAR_SUMMARY}"
printf 'CUDA_VISIBLE_DEVICES=%s\n' "${CUDA_VISIBLE_DEVICES}"
printf 'NUM_PROCESSES=%s\n' "${NUM_PROCESSES}"
printf 'BATCH_PER_GPU=%s\n' "${BATCH_PER_GPU}"
printf 'GLOBAL_BATCH_SIZE=%s\n' "${GLOBAL_BATCH_SIZE}"
printf 'STEPS=%s\n' "${STEPS}"
printf 'SAVE_FREQ=%s\n' "${SAVE_FREQ}"
printf 'HIVA_DECODED_ACTION_LOSS_WEIGHT=%s\n' "${HIVA_DECODED_ACTION_LOSS_WEIGHT}"
printf 'HIVA_DECODED_ROT_LOSS_WEIGHT=%s\n' "${HIVA_DECODED_ROT_LOSS_WEIGHT}"
printf 'HIVA_DECODED_TR_LOSS_BETA=%s\n' "${HIVA_DECODED_TR_LOSS_BETA}"
printf 'HIVA_DECODED_ROT_LOSS_BETA=%s\n' "${HIVA_DECODED_ROT_LOSS_BETA}"
printf 'HIVA_DECODED_GRIP_LOSS_BETA=%s\n' "${HIVA_DECODED_GRIP_LOSS_BETA}"
printf 'HIVA_RESIDUAL_ENABLED=%s\n' "${HIVA_RESIDUAL_ENABLED}"
printf 'HIVA_RESIDUAL_MODE=%s\n' "${HIVA_RESIDUAL_MODE}"
printf 'HIVA_RESIDUAL_SCALE_MULT=%s\n' "${HIVA_RESIDUAL_SCALE_MULT}"
printf 'HIVA_TRAIN_RESIDUAL_ONLY=%s\n' "${HIVA_TRAIN_RESIDUAL_ONLY}"
