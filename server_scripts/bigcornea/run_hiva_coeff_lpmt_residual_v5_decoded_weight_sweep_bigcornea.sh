...
# Standalone bigcornea LP-MT HiVA residual sweep for v5 d4/6/10 sidecars.
#
# This script intentionally does not call the shared finetune/common wrappers.
# It runs four jobs sequentially, sweeping HIVA_DECODED_ACTION_LOSS_WEIGHT.
...
LOG_FILE="${LOG_FILE:-${LOG_DIR}/run_hiva_coeff_lpmt_residual_v5_decoded_weight_sweep_bigcornea_${RUN_STAMP}.log}"
...
HIVA_RESIDUAL_MODE="${HIVA_RESIDUAL_MODE:-token_to_time}"
...
# Empty scale overrides mean: use ACTION std from dataset/config stats.
...
if [[ -n "${HIVA_DURATION_MEAN}" ]]; then
  OPTIONAL_ARGS+=(--policy.hiva_duration_mean="${HIVA_DURATION_MEAN}")
fi
if [[ -n "${HIVA_DURATION_STD}" ]]; then
  OPTIONAL_ARGS+=(--policy.hiva_duration_std="${HIVA_DURATION_STD}")
fi
...
WEIGHTS_RAW="${HIVA_DECODED_ACTION_LOSS_WEIGHTS:-0.5 1.0 1.5 2.0}"
read -r -a DECODED_ACTION_WEIGHTS <<< "${WEIGHTS_RAW}"

echo "Standalone LP-MT residual v5 decoded-loss sweep started at $(date)"
...
echo "HIVA_RESIDUAL_MODE=${HIVA_RESIDUAL_MODE}"
echo "HIVA_RESIDUAL_SCALE_MULT=${HIVA_RESIDUAL_SCALE_MULT}"
echo "HIVA_RESIDUAL_HORIZON=${HIVA_RESIDUAL_HORIZON}"
echo "HIVA_DECODED_ROT_LOSS_WEIGHT=${HIVA_DECODED_ROT_LOSS_WEIGHT}"
echo "HIVA_DECODED_ACTION_LOSS_WEIGHTS=${DECODED_ACTION_WEIGHTS[*]}"

run_one_weight() {
  local decoded_weight="$1"
  local weight_tag="${decoded_weight//./p}"
  local run_name="${RUN_NAME_PREFIX:-smolvla_hiva_coeff_lpmt_residual_v5_d4_6_10_coeffpool_full_ce_mean_k10_f15_bigcornea_b${BATCH_PER_GPU}_s${S}}_daw${weight_tag}_${RUN_STAMP}"
  local output_dir="${OUTPUT_DIR_ROOT:-${REPO_ROOT}/outputs/train}/${run_name}"
  ...
  echo "===== Starting LP-MT residual v5 decoded weight=${decoded_weight} at $(date) ====="
  ...
  TRAIN_ARGS=(
    ...
    --policy.hiva_decoded_action_loss_weight="${decoded_weight}"
  ...
  echo "===== Finished LP-MT residual v5 decoded weight=${decoded_weight} at $(date) ====="
}

for decoded_weight in "${DECODED_ACTION_WEIGHTS[@]}"; do
  run_one_weight "${decoded_weight}"
done

echo "Standalone LP-MT residual v5 decoded-loss sweep finished at $(date)"
